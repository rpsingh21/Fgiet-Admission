...
class HighSchool(models.Model):
    candidate = models.OneToOneField(
        Candidate, on_delete=models.CASCADE, related_name="hs")
    highSchoolPassingYear = models.CharField(
        max_length=16, choices=passing_year)
...
class Intermediate(models.Model):
    candidate = models.OneToOneField(
        Candidate, on_delete=models.CASCADE, related_name="inter")
    intermediatePassingYear = models.CharField(
        max_length=16, choices=passing_year)
...
class PCM(models.Model):
    intermediate = models.OneToOneField(
        Intermediate, on_delete=models.CASCADE, related_name="pcm")
...
class UgOrDiploma(models.Model):
    candidate = models.OneToOneField(
        Candidate, on_delete=models.CASCADE, related_name="ug")
    ugOrDiplompassingYear = models.CharField(
        max_length=16, choices=passing_year)
...
class Upsee(models.Model):
    candidate = models.OneToOneField(
        Candidate, on_delete=models.CASCADE, related_name="upsee")
...
class Branch(models.Model):
    candidate = models.OneToOneField(
        Candidate, on_delete=models.CASCADE, related_name="cbranch")
    first = models.CharField(max_length=64, choices=branch_choices)
    second = models.CharField(
        max_length=64, choices=branch_choices, blank=True, null=True)
