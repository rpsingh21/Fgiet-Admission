...
def form_view(request, year, course):
    ...
    if request.method == 'POST':
        ...
        vaild = True
        if not (
                candidateForm.is_valid() and highSchoolForm.is_valid() and
                upseeForm.is_valid):
            vaild = False
        ...
        if vaild:
            ...
            instance.registrationNo = "21187"+"{:04}".format(instance.id)
            instance.save()
            ...
            mgs = "Dear! "+instance.name+", You're registered successfully and \
                Reg No is " + \
                instance.registrationNo+". For more Information \
                    visit website http://www.fgiet.ac.in"
...
def success_view(request):
    ...
    return render(
        request, 'success.html', {'instance': instance, 'applicant': True})
...
def application_view(request, id):
    ...
    if not request.user.is_superuser:
        if request.user.profile.branch != instance.course:
            return HttpResponse(
                "User is not vaild to access this application details")
